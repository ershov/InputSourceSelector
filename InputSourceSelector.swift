...
func printInputSourceName(_ inputSource: TISInputSource) {
...
if argc > 1 && (argv[1] == "list" || argv[1] == "list-enabled") {
    let listAll = argv[1] == "list"
    let inputSources = TISCreateInputSourceList(nil, listAll).takeUnretainedValue() as NSArray as! [TISInputSource]
    for inputSource in inputSources {
        printInputSourceName(inputSource)
    }
} else if argc > 1 && (argv[1] == "current" || argv[1] == "current-layout") {
    let inputSourceUnmanaged = argv[1] == "current-layout" ? TISCopyCurrentKeyboardLayoutInputSource() : TISCopyCurrentKeyboardInputSource()
